...
async def _process_document_async(
    ...
    try:
        ...
        # Step 1: Initialize DB client and idempotency guard
        logger.debug(
            f"Initializing database client and idempotency check for topic_name={topic_name}")
        ...
        task_name = "tasks.document_tasks.document_upload_and_process"
        task_time_limit = getattr(
            celery_app.conf, "task_time_limit", 600) or 600
        ...
        if not should_execute:
            logger.warning(
                f"Skipping execution due to idempotency guard | task_name={task_name} | "
                f"status={existing_task.status if existing_task else 'UNKNOWN'}"
            )
            return existing_task.result if existing_task else {"success": False, "message": "duplicate_task"}
        ...
        # Step 2: Initialize external clients after idempotency passes
        logger.debug(
            f"Initializing vector DB and embedding clients for topic_name={topic_name}")
        vectordb_factory = VectorDBProviderFactory(config)
        vectordb_client = vectordb_factory.create(
            config.vector_db_type.upper())
        ...
        logger.info(
            f"Validating uploaded file | filename={filename} | topic_name={topic_name}")
        ...
        # Step 6: Generate unique filepath
        filepath_start = time.time()
        logger.debug(
            f"Generating unique filepath | filename={filename} | topic_name={topic_name}")
        ...
        logger.info(
            f"Saving file to disk | file_id={file_id} | file_path={file_path}")
        ...
        # Step 8: Determine document type
        ...
        logger.debug(
            f"Document type determined | file_id={file_id} | type={document_type}")

        # Step 9: Create document record in database
        db_create_start = time.time()
        logger.info(
            f"Creating document record in database | file_id={file_id} | topic_name={topic_name}")
        ...
        # Step 10: Chunk the document and store chunks in DB
        ...
        # Step 11: Generate embeddings and index into vector DB
